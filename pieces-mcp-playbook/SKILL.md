...
Always supported:
- `question` (string)
- `chat_llm` (string): e.g., `"gemini-2.5-flash"`. This parameter is often *required* by the MCP host even if schemas mark it optional. Always include.
...
2.  If `mcporter` isn't finding the server at all, explicitly add it: 
    `mcporter config add pieces http://192.168.86.34:39300/model_context_protocol/2025-03-26/mcp --allow-http` (adjust URL if needed).
